...
test('Not a existing user request /v1/all/:btag', async () => {
    ...
    const rp = await request.get('/v1/all/not_existinguserxd')
        .expect(400)
...
test('Not a existing user request /v1/all/:btag/:platform', async () => {
    ...
    const rp = await request.get('/v1/all/not_existinguserxd/pc')
        .expect(400)
        .expect('content-type', 'application/json; charset=utf-8');
...
test('Tell not a valid platform /v1/all/:btag/notvalidplatform', async () => {
    const rp = await request.get('/v1/all/not_existinguserxd/notvalidplatform')
        .expect(400)
        .expect('content-type', 'application/json; charset=utf-8');
